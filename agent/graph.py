...
def _extract_json(text: str) -> dict[str, Any]:
    """Parse a small JSON object from an LLM reply."""
    fenced = re.search(r"```(?:json)?\s*(\{.*?\})\s*```", text, re.DOTALL | re.IGNORECASE)
    blob = fenced.group(1) if fenced else text
    start = blob.find("{")
    end = blob.rfind("}")
    if start == -1 or end == -1 or end < start:
        return {"ok": False, "issue": "verifier returned non-JSON"}
    try:
...
def verify_node(state: AgentState) -> dict:
    ...
    # Fast-path obvious failures without an LLM call.
    if not execution.ok:
...
def revise_node(state: AgentState) -> dict:
    ...
    response = llm().invoke([
        ...
        ("user", prompts.REVISE_USER.format(
            ...
            question=state.question,
            sql=state.sql,
            execution=execution.render() if execution else "ERROR: no execution",
            issue=state.verify_issue or "result not plausible",
        )),
    ])
    sql = _extract_sql(response.content)
    return {
